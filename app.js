...
// Network canvas background (blue network)
(function () {
    ...
    const particleColor = 'rgba(100, 180, 255, 0.6)';
    const lineColor = 'rgba(100, 180, 255, 0.15)';
    ...
    class NetworkParticle {
        constructor() {
            ...
            this.vx = (Math.random() * 2 - 1) * particleSpeed;
            this.vy = (Math.random() * 2 - 1) * particleSpeed;
        }
        update() {
    ...
    function initParticles() {
        particles = [];
        const n = calculateNumberOfParticles();
        for (let i = 0; i < n; i++) particles.push(new NetworkParticle());
    }

    function connectParticles() {
        const match = lineColor.match(/,\s*(\d?\.?\d*)\)/);
        const baseAlpha = match ? parseFloat(match[1]) : 0.15;
        for (let a = 0; a < particles.length; a++) {
            for (let b = a + 1; b < particles.length; b++) {
                ...
                if (dist < connectDistance) {
                    const alpha = (1 - dist / connectDistance) * baseAlpha;
                    ctx.strokeStyle = lineColor.replace(/,\s*\d?\.?\d*\)/, `, ${alpha})`);
                    ctx.lineWidth = 1;
                    ctx.beginPath();
        ...
        if (mouse.x !== null && mouse.y !== null) {
            for (let i = 0; i < particles.length; i++) {
                ...
                if (dist < mouse.radius) {
                    const alpha = Math.min(0.4, (1 - dist / mouse.radius) * baseAlpha * 2.5);
                    ctx.strokeStyle = lineColor.replace(/,\s*\d?\.?\d*\)/, `, ${alpha})`);
                    ctx.lineWidth = 1;
                    ctx.beginPath();
    ...
    window.addEventListener('resize', () => {
        setSize();
        mouse.radius = Math.min(150, Math.max(50, canvas.width / 10));
        initParticles();
    });

    initParticles();
    animate();
    console.log('Network canvas background initialized');
})();